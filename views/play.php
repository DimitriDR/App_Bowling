...
<main class="container mx-auto border rounded-md px-10 py-5 mt-6">
    ...
    <h2 class="text-4xl text-center font-light">Tour <?= $game->get_current_round(); ?> – <?= htmlspecialchars($game->get_current_player_name()) ?></h2>
        <form method="POST" action="/controllers/play.php" class="grid grid-cols-1 gap-4 content-center mx-auto">
            <label for="throw_value" class="text-xl mt-5 text-center font-semibold">Saisir le score du lancer n°<?= "X" ?></label>
            <input class="mx-auto w-80 block rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" type="number" name="throw_value" id="throw_value" min="1" max="10" required>
